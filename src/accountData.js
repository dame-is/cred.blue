 ...
 *
 * This revised implementation no longer uses the old onPage callback.
 * Instead, it calls onAPICall after each API request, so we can track progress based on
 * the number of API calls for listRecords.
 *
 * @param {string} collectionName - The collection name.
 * @param {number} expectedPages - Expected number of pages (for progress calculation).
 * @param {Function} onAPICall - A callback that receives the current API call number.
 * @returns {Promise<Array>} - The complete list of records.
 */
async function fetchRecordsForCollection(collectionName, expectedPages = 50, onAPICall = () => {}) {
 ...
 * This revised implementation no longer uses the old onPage callback.
 * Instead, it calls onAPICall after each API request, so we can track progress based on
 * the number of API calls for getAuthorFeed.
 *
 * @param {number} expectedPages - Expected number of pages (for progress calculation).
 * @param {Function} onAPICall - A callback that receives the current API call number.
 * @returns {Promise<Array>} - The complete author feed.
 */
async function fetchAuthorFeed(expectedPages = 10, onAPICall = () => {}) {
...
async function calculateRecordsAggregate(collectionNames, ageInDays) {
  ...
  for (const col of collectionNames) {
    ...
    const recs = await fetchRecordsForCollection(col, 50, () => {}); // No progress update here
...
async function calculateEngagements() {
  // Using our revised getAuthorFeed which will call onAPICall after each fetch.
  const feed = await fetchAuthorFeed(15, () => {}); // No progress update here as progress is handled below.
  let likesReceived = 0;
...
export async function loadAccountData(inputHandle, onProgress = () => {}) {
  try {
    ...
    // --- Define a new progress methodology that focuses on the two slow endpoints ---
    // We now assign a total of 100% progress in which 50% is allotted for all listRecords API calls
    // and 50% is allotted for all getAuthorFeed API calls.
    // The onAPICall callback in each function will provide the current call number and we use the expected number
    // of pages (passed as expectedPages) to compute the incremental progress update.
    const updateAPICallProgress = (callNumber, expectedPages, weight) => {
      // weight should be 0.5 (50% overall progress) in our case
      const inc = (1 / expectedPages) * weight;
      onProgress(inc * 100);
    };
    ...
    // 8. Detailed post statistics using listRecords for "app.bsky.feed.post"
    // Instead of using the old onPage progress,
    // we now track progress based on the number of API calls.
    const postsRecords = await fetchRecordsForCollection(
      "app.bsky.feed.post",
      20,
      (callNumber) => { updateAPICallProgress(callNumber, 20, 0.5); }
    );
    ...
    const repostRecords = await fetchRecordsForCollection(
      ...
      10,
      (callNumber) => { updateAPICallProgress(callNumber, 10, 0); } // We don’t add repost calls to progress (or you can change weight)
    );
    ...
    // End of detailed post stats phase.
    ...
    // 10. Compute engagements using getAuthorFeed.
    // Here we track progress for getAuthorFeed API calls.
    const engagementsReceived = await fetchAuthorFeed(
      15,
      (callNumber) => { updateAPICallProgress(callNumber, 15, 0.5); }
    );
    // Then process engagements as before.
